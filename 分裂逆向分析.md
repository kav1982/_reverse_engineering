...
> 配套：总览见《魔法工艺法术系统逆向报告.md》，原始数值见《法术数值总表.md》，悬停结束才分裂见《悬停逆向分析.md》，闪电链分裂弹系数见《闪电链逆向分析.md》，注魔硬币分裂补丁见《注魔硬币逆向分析.md》5.3。
...
| 注魔硬币 / 魔力金币 | 抄活体 `CoinUseCount → Int3`、`BuffRatio → Float3`。子弹 init **不再扣金**（`num=0`），改为 `Damage.AddRatio += Int3 × Float2 / 100`。母弹销毁当帧先吐钱再裂，子弹 `CoinUseCount=0` 不掉。`BuffRatio` 恒 0、`Float3` 无人读。金币加算已在被复印的 `Damage` 里，再叠一层比率是双重计算。详见《注魔硬币逆向分析.md》5.3 / 7.10 |
...
注魔硬币这一行原先只写「抄活体 `CoinUseCount`、`BuffRatio`」，会让人以为子弹原样继承扣金计数、`BuffRatio` 有用。两边都不对：抄过去的是快照字段，子弹用的是另一条 `AddRatio` 公式；`BuffRatio` 是死字段。落地金币由 `Spell1020GroundCoinSpawnerSystem` 在 `SpellDestroySystem`（`OrderLast`）之前生成，所以钱只从母弹吐一次。
