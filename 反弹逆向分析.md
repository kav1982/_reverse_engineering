...
不走 `SpellMoveJob` 墙撞（龙息直接被 Job 排除；激光用自己的步进 Job）。`ReboundCount` 被读成**折线段数**：

- 无视墙壁：墙检测直接 return，次数不扣。
- 否则撞墙：`ReboundCount--`（分解射线抄到临时字段 `rebounceTime` 再扣），`stepDirection = math.reflect(...)`。
- 次数为 0 再撞墙：线段结束。

折射仍走单位命中，和墙折线分开。这里不展开完整射线算法。
