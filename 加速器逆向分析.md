...
> 配套：总览见《魔法工艺法术系统逆向报告.md》，原始数值见《法术数值总表.md》，加算容器见《伤害强化逆向分析.md》，子弹速度再 ×0.5 见《分裂逆向分析.md》，`speed = 0` 法术被加成后的形态改变见《落雷阵逆向分析.md》4.2 / 7.6。
...
### 3.4 `speed = 0` 的法术：改的是形态，不是数值

**`Speed.Base = 0` 的法术会被加速器从「静止」直接变成「移动」。** 这不是一档数值加成，而是形态改变，收益判定得单独做，不能套 3.3 那张「速度越高走得越远越好」的表。

`Calculate()` 里 `Base` 和 `AddBase` 是简单相加，没有「`Base` 为 0 就短路」这种保护，链路一步没少：

```
Speed = (0 + Σfloat1) × (1 + AddRatio) × MulRatio + Extra = Σfloat1
```

`GetSpellFlySpeed` 的 `AddBase(float1)`（`SpellShootData.cs` L86–95）→ `sip.bounsSpeed`（`SpellInitialParameter.cs` L307）→ `Speed.AddBase`（`ShootSpellUtils.cs` L201）→ `Calculate()`。

**实例：落雷阵 `1018`**（`speed = 0`、`duration = 5`、`radius = 2.8/3.0/3.2`）。裸装是一个钉在法杖枪口的静止光环，每 0.25 秒对半径内最多 `int1` 个敌人各劈一次。它在 `SpellMoveJob` 里没有任何专属分支，走 `Normal` → `Normal_Common`，所以加速器一插就成了一朵沿施法方向匀速直线漂移的雷云：


| 槽位 | `Movement.Speed` | 5 秒漂移距离 | 结果 |
| --- | --- | --- | --- |
| `[落雷阵]` | 0 | 0 m | 钉在枪口 |
| `[加Lv1(+3)] [落雷阵]` | **3** | 15 m | 横穿大半个房间 |
| `[加Lv2(+6)] [落雷阵]` | **6** | 30 m | 一般在撞墙前就飞出交战区 |
| `[加Lv3(+12)] [落雷阵]` | **12** | 60 m | 基本等于把光环丢掉 |


**判定：对落雷阵是负收益，而且等级越高越差。** 落雷阵的价值在于封住一小片区域让敌人持续吃伤；让它匀速直线飘走会大幅缩短它停在敌人身上的时间。这和加速器对几乎所有其它法术的定位正好相反，是全表少见的「加速器插了变弱」的场景。

这是设计内行为，不是漏网。`SpellGroupAttackDistanceCalculator` 给 `ThunderAura` 单独写了一条用 `max` 的分支：
...
裸装 `max(2.8, 0 × 5) = 2.8`，插 Lv1 后 `max(2.8, 3 × 5) = 15`。策划已经预料到这类法术的速度可以非 0，并给自动施法法杖 / 队友 AI 的开火距离做了适配。

**缓解方案：配悬停。** `SpellSystem.UpdateDuration` 在 `duration` 用尽后**先把 `Speed` 压 0** 再进悬停段（`SpellSystem.cs` L287），而落雷阵在悬停期仍以完全相同的频率结算。所以 `[加速器] [悬停] [落雷阵]` 是「推进到位再驻扎」：光环先飘到目标区域，5 秒到点后停下来继续输出，比单插加速器合理得多。

**同类：黑洞 `1007`。** 底数不是 0，是 `1.2`，但判定完全一样——价值在把人留在圈里持续吃伤。加算的相对量极大：

| 槽位 | `Movement.Speed` | 5 秒位移 | 结果 |
| --- | --- | --- | --- |
| `[黑洞]` | 1.2 | 6 m | 几乎钉在落点附近 |
| `[加Lv1(+3)] [黑洞]` | **4.2** | 21 m | 横穿大半个房间 |
| `[加Lv3(+12)] [黑洞]` | **13.2** | 66 m | 基本等于把圈丢掉 |

`Spell1007Job` 不读速度，DPS / 牵引跟着实体走；实体飞走，圈也就走了。缓解同样是配悬停。AI 射程估算走 `speed × duration`，会跟着加速器线性放大。详见《黑洞逆向分析.md》7.5。

**通用限定**：核对 `speed = 0`（以及「底数极低、靠驻场吃饭」）的法术时，加速器要按「它会不会离开你想让它待的地方」来判。同一条 `AddBase` 上的寻踪（`3010`，见 4.1）量级只有 1 / 2 / 4 且方向跟鼠标，在「把静止法术推到该去的位置」这个需求上通常优于加速器。
...
| `SpellMoveJob` | 普通弹 `Direction × Speed`；`Normal_Common` 让 `speed = 0` 的法术在插卡后开始漂移 |
| `SpellSystem.UpdateDuration`（L287） | `duration` 用尽后先把 `Speed` 压 0，再进悬停段——「推进到位再驻扎」的机制来源 |
| `SpellGroupAttackDistanceCalculator`（L283–285） | `ThunderAura` 分支写 `max(半径, 速度 × 时长)`，证明 `speed = 0` 被加成是设计内行为 |
