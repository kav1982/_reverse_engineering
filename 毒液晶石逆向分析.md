...
落雷阵是 `isDPS = true`、`DPSDamageInterval = 0.25`、`speed = 0` 的原地光环：一次施放**只有一个实体**，但它活满 `duration = 5` 秒、每 0.25 秒结算一次，对站在圈里不走的同一个目标就是 **20 次独立命中**，每次各叠一份 `int2`。再叠时长强化 Lv3（44 次结算）和悬停 Lv3（悬停期光环仍全速结算），单目标可以堆到 **256 层**。详见《落雷阵逆向分析.md》7.14。

黑洞是同一条 `isDPS` 管线（`interval=0.25`、满 5 s ≈ 20 tick），单目标叠层和落雷阵持平。差别在两边：落雷阵同时打几人钳在 `int1`，黑洞**不封顶**；落雷阵靠站桩留人，黑洞每 tick 按 `float2 × 0.25` 往圆心拉，会走的敌人更容易留在圈里。两者都每 tick 调 `GetSpellElementDataWithTimeScale`。tick 类并列最强，场景不同——多目标和移动敌人上黑洞更占便宜。详见《黑洞逆向分析.md》7.10。
