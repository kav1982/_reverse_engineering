...
排除三种自己管落雷的法术：`LaserBeam`（1004）、`DaveHarpoons`（4024）、`BiAnLethalBlade`（4019）。
...
- 搜敌：敌对阵营 + 脆弱物，半径用已经乘完范围的 `ThunderHitRadius`。
...
激光 1004 被 Job 排除，槽位上的 3101 **不会**走这套销毁落雷。
...
| --- | --- | --- |
| 激光 1004 | 烘焙了，但销毁 Job **排除** `LaserBeam` | 这套圈伤基本不触发。处理器里 `ThunderHitChance>0` 且颜色不是 `Player` 时会强行涂雷（`Random.Range(0,1)==0` 对整数恒真），只改外观 |
| 激光晶石 4014 | 半径 / 伤害比用 Element；概率用整杖 `Max(float3)` 写入 `ThunderStoneRate` | 命中帧 `NextInt(100) < Rate` 时 `GenerateThunderDamage`（**不看** `ColorType`） |
...
| `SpellDestroyEventJob` | 通用销毁调 `CheckFallThunderDamage`；排除 1004 / 4024 / 4019 |
