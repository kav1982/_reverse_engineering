...
[CompilerGenerated]
[BurstCompile]
public struct SpellDestroyEventJob : IJobEntity, IJobChunk
{
	public struct InternalCompilerQueryAndHandleData
	{
		...
		private void __AssignQueries(ref SystemState state)
		{
			...
			EntityQueryBuilder entityQueryBuilder2 = entityQueryBuilder.WithAllRW<SpellDestroyTag>();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Run(ref SpellDestroyEventJob job, EntityQuery query)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle Schedule(ref SpellDestroyEventJob job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle ScheduleParallel(ref SpellDestroyEventJob job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void UpdateBaseEntityIndexArray(ref SpellDestroyEventJob job, EntityQuery query, ref SystemState state)
		{
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle UpdateBaseEntityIndexArray(ref SpellDestroyEventJob job, EntityQuery query, JobHandle dependency, ref SystemState state)
		{
			return dependency;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void AssignEntityManager(ref SpellDestroyEventJob job, EntityManager entityManager)
		{
		}
	}
	...
	[BurstCompile]
	private void Execute(SpellAspect spell, [ChunkIndexInQuery] int chunkIndex, SpellDestroyTag _)
	{
	...
	public void Execute(in ArchetypeChunk chunk, int chunkIndexInQuery, bool useEnabledMask, in v128 chunkEnabledMask)
	{
		...
		if (!useEnabledMask)
		{
			for (int i = 0; i < count; i++)
			{
				...
				Execute(spell, chunkIndexInQuery, default(SpellDestroyTag));
				num++;
			}
			return;
		}
		if (math.countbits(chunkEnabledMask.ULong0 ^ (chunkEnabledMask.ULong0 << 1)) + math.countbits(chunkEnabledMask.ULong1 ^ (chunkEnabledMask.ULong1 << 1)) - 1 <= 4)
		{
			...
			while (InternalCompilerInterface.UnsafeTryGetNextEnabledBitRange(chunkEnabledMask, nextRangeEnd, out nextRangeBegin, out nextRangeEnd))
			{
				while (nextRangeBegin < nextRangeEnd)
				{
					...
					Execute(spell2, chunkIndexInQuery, default(SpellDestroyTag));
					nextRangeBegin++;
					num++;
				}
			}
		...
		for (int j = 0; j < num3; j++)
		{
			if ((num2 & 1) != 0L)
			{
				...
				Execute(spell3, chunkIndexInQuery, default(SpellDestroyTag));
				num++;
			}
			num2 >>= 1;
		}
		...
		for (int k = 64; k < count; k++)
		{
			if ((num2 & 1) != 0L)
			{
				...
				Execute(spell4, chunkIndexInQuery, default(SpellDestroyTag));
				num++;
			}
			num2 >>= 1;
		}
