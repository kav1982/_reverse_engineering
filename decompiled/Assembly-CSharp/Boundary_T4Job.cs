...
[BurstCompile]
[CompilerGenerated]
public struct Boundary_T4Job : IJobEntity, IJobChunk
{
	public struct InternalCompilerQueryAndHandleData
	{
		...
		{
			[ReadOnly]
			public ComponentTypeHandle<Boundary_T4_Dots> __Boundary_T4_Dots_RO_ComponentTypeHandle;
			...
			{
				__Boundary_T4_Dots_RO_ComponentTypeHandle = state.GetComponentTypeHandle<Boundary_T4_Dots>(isReadOnly: true);
			...
			{
				__Boundary_T4_Dots_RO_ComponentTypeHandle.Update(ref state);
		...
		private void __AssignQueries(ref SystemState state)
		{
			...
			EntityQueryBuilder entityQueryBuilder2 = entityQueryBuilder.WithAll<Boundary_T4_Dots>();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Run(ref Boundary_T4Job job, EntityQuery query)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle Schedule(ref Boundary_T4Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle ScheduleParallel(ref Boundary_T4Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void UpdateBaseEntityIndexArray(ref Boundary_T4Job job, EntityQuery query, ref SystemState state)
		{
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle UpdateBaseEntityIndexArray(ref Boundary_T4Job job, EntityQuery query, JobHandle dependency, ref SystemState state)
		{
			return dependency;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void AssignEntityManager(ref Boundary_T4Job job, EntityManager entityManager)
		{
		}
	}
	...
	public void Execute([ChunkIndexInQuery] int index, in Boundary_T4_Dots boundary, in BoundaryBase_Dots boundaryBase, Entity ett)
	{
		...
		cluLocalTsf.GetRefRW(boundary.ett_LayerWall).ValueRW.Position = DTool.GetLayerPosition(in refRW.ValueRW.Position, LayerCorrectType.Coordinate);
		...
		else if (flag && flag3 && flag5 && flag7)
		{
			...
			{
				refRW.ValueRW.Position += new float3(0f, 0f, -0.001f);
				flag14 = true;
			}
			else if (!flag6)
			{
		...
		ecb.SetComponentEnabled<Boundary_T4_Dots>(index, ett, value: false);
	}

	private void RandomDetail(int index, Boundary_T4_Dots boundary, BoundaryBase_Dots boundaryBase, Entity createEtt, bool flip)
	{
	...
	{
		IntPtr nativeArrayPtr = InternalCompilerInterface.UnsafeGetChunkNativeArrayReadOnlyIntPtr(chunk, ref __TypeHandle.__Boundary_T4_Dots_RO_ComponentTypeHandle);
		...
		if (!useEnabledMask)
		{
			...
			{
				ref Boundary_T4_Dots boundary = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T4_Dots>(nativeArrayPtr, i);
		...
		if (math.countbits(chunkEnabledMask.ULong0 ^ (chunkEnabledMask.ULong0 << 1)) + math.countbits(chunkEnabledMask.ULong1 ^ (chunkEnabledMask.ULong1 << 1)) - 1 <= 4)
		{
			...
			while (InternalCompilerInterface.UnsafeTryGetNextEnabledBitRange(chunkEnabledMask, nextRangeEnd, out nextRangeBegin, out nextRangeEnd))
			{
				...
				{
					ref Boundary_T4_Dots boundary2 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T4_Dots>(nativeArrayPtr, nextRangeBegin);
		...
		for (int j = 0; j < num3; j++)
		{
			if ((num2 & 1) != 0L)
			{
				ref Boundary_T4_Dots boundary3 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T4_Dots>(nativeArrayPtr, j);
		...
		for (int k = 64; k < count; k++)
		{
			if ((num2 & 1) != 0L)
			{
				ref Boundary_T4_Dots boundary4 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T4_Dots>(nativeArrayPtr, k);
