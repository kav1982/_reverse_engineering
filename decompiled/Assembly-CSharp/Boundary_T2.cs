...
public class Boundary_T2 : BoundaryBase
{
	[Space(50f)]
	...
	[Header("Shaodow")]
	public GameObject go_ShadowFull;

	public GameObject go_ShadowBig;

	public GameObject go_ShadowUp;

	public GameObject go_ShadowRight;
	...
	public override void Correct(Vector2Data selfPoint, RoomController roomCtrller)
	{
		...
		go_ShadowFull.transform.position = Tool2D.GetLayerPoint(base.transform, LayerCorrectType.Shadow);
		go_ShadowBig.transform.position = Tool2D.GetLayerPoint(base.transform, LayerCorrectType.Shadow);
		go_ShadowUp.transform.position = Tool2D.GetLayerPoint(base.transform, LayerCorrectType.Shadow);
		go_ShadowRight.transform.position = Tool2D.GetLayerPoint(base.transform, LayerCorrectType.Shadow);
		bool flag = false;
		bool flag2 = false;
		bool flag3 = false;
		bool flag4 = false;
		...
		if (flag && !flag3 && !flag5 && !flag7)
		{
			...
			go_ShadowUp.SetActive(value: true);
		...
		else if (flag && flag3 && !flag5 && !flag7)
		{
			...
			{
				Object.Instantiate(ABResources.LoadAsset<GameObject>((roomCtrller.roomCfg.themeType == RoomThemeType.Theme2_Chapter2_Oblique) ? "Prefabs/Scene/Theme0/Boundary" : "Prefabs/Scene/Theme18/Boundary"), base.transform.position, Quaternion.identity, base.transform.parent).GetComponent<Boundary_T0>().Correct(selfPoint, roomCtrller);
			}
			else
			{
				...
				go_ShadowBig.SetActive(value: true);
		...
		else if (flag && !flag3 && !flag5 && flag7)
		{
			...
			else
			{
				...
				go_ShadowBig.SetActive(value: true);
				go_ShadowBig.transform.rotation = Tool2D.GetRotation(90f);
		...
		else if (flag && flag3 && !flag5 && flag7)
		{
			...
			go_ShadowFull.SetActive(value: true);
