...
public class Boss50Collider : UnitBase
{
	public List<Entity> hitList = new List<Entity>();

	public Boss50 master;
	...
	public unsafe void Init(Boss50 master, float maxHp)
	{
		this.master = master;
		UnitProperty_Dots componentData = GetComponentData<UnitProperty_Dots>();
		...
		geometry.Size = thisCollider.size;
		geometry.Center = thisCollider.center;
		colliderPtr->Geometry = geometry;
		SetComponentData(componentData2);
		componentData.unitCfg.maxHP = maxHp;
		componentData.unitCfg.currentHP = maxHp;
