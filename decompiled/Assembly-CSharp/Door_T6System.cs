...
public struct Door_T6System : ISystem, ISystemCompilerGenerated
{
	[StructLayout(LayoutKind.Sequential, Size = 1)]
	private readonly struct IFE_1949752912_0
	{
		...
		public struct TypeHandle
		{
			...
			{
				item1_ComponentTypeHandle_RW = systemState.GetComponentTypeHandle<Door_T6_Dots>();
		...
		{
			state.EntityManager.CompleteDependencyBeforeRW<Door_T6_Dots>();
	...
	{
		public IFE_1949752912_0.TypeHandle __IFE_1949752912_0_TypeHandle;
		...
		{
			__IFE_1949752912_0_TypeHandle = new IFE_1949752912_0.TypeHandle(ref state);
			__Unity_Transforms_LocalTransform_RW_ComponentLookup = state.GetComponentLookup<LocalTransform>();
		}
	}

	[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
	internal delegate void __codegen__OnCreate_00005A57_0024PostfixBurstDelegate(IntPtr self, IntPtr state);

	internal static class __codegen__OnCreate_00005A57_0024BurstDirectCall
	{
		...
		private static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = BurstCompiler.CompileFunctionPointer<__codegen__OnCreate_00005A57_0024PostfixBurstDelegate>(delegate(IntPtr self, IntPtr state)
				{
					Invoke(self, state);
				}).Value;
			}
	...
	private EntityQuery __query_1949752912_0;

	private EntityQuery __query_1949752912_1;

	[BurstCompile]
	public void OnCreate(ref SystemState state)
	{
		state.RequireForUpdate<EndSimulationEntityCommandBufferSystem.Singleton>();
		state.RequireForUpdate<Door_T6_Dots>();
	}
	...
	public void OnUpdate(ref SystemState state)
	{
		...
		foreach (QueryEnumerableWithEntity<InternalCompilerInterface.UncheckedRefRW<Door_T6_Dots>, InternalCompilerInterface.UncheckedRefRW<PhysicsCollider>, InternalCompilerInterface.UncheckedRefRW<DoorBase_Dots>, LocalTransform, InternalCompilerInterface.UncheckedRefRW<InteractiveObj_Dots>> item6 in IFE_1949752912_0.Query(__query_1949752912_0, __TypeHandle.__IFE_1949752912_0_TypeHandle, ref state))
		{
			item6.Deconstruct(out var item, out var item2, out var item3, out var item4, out var item5, out var entity);
			InternalCompilerInterface.UncheckedRefRW<Door_T6_Dots> uncheckedRefRW = item;
			...
			if (!uncheckedRefRW.ValueRW.isInitialized)
			{
				...
				EntityCommandBuffer ecb = __query_1949752912_1.GetSingleton<EndSimulationEntityCommandBufferSystem.Singleton>().CreateCommandBuffer(state.WorldUnmanaged);
				uncheckedRefRW2.ValueRW.MakeUnique(in entity2, ecb);
				float3 layerPosition = DTool.GetLayerPosition(in localTransform.Position, LayerCorrectType.AccessOpen);
				...
				switch (uncheckedRefRW.ValueRW.themeType)
				{
				default:
					uncheckedRefRW.ValueRW.doorSpineMono.Value = ObjPoolMgr.Inst.GetGO("Prefabs/Mixed/Door_T6Mono").GetComponent<Door_SpineMono>();
					break;
				case RoomThemeType.Theme12_Chapter5_2:
					uncheckedRefRW.ValueRW.doorSpineMono.Value = ObjPoolMgr.Inst.GetGO("Prefabs/Mixed/Door_T12Mono").GetComponent<Door_SpineMono>();
					break;
				case RoomThemeType.Theme22_Chapter3_Shortcut1:
					uncheckedRefRW.ValueRW.doorSpineMono.Value = ObjPoolMgr.Inst.GetGO("Prefabs/Mixed/Door_T22Mono").GetComponent<Door_SpineMono>();
					break;
				case RoomThemeType.Theme21_Chapter5_Store:
					break;
				}
				if (uncheckedRefRW.ValueRW.doorSpineMono.Value != null)
				{
					uncheckedRefRW.ValueRW.doorSpineMono.Value.transform.position = localTransform.Position + layerPosition;
				}
			}
			...
			if (uncheckedRefRW3.ValueRW.onUpdateDisplay)
			{
				...
				if (uncheckedRefRW3.ValueRW.rewardType == LevelRewardType.Ruined)
				{
					if (uncheckedRefRW.ValueRW.doorSpineMono.Value != null)
					{
						uncheckedRefRW.ValueRW.doorSpineMono.Value.gameObject.SetActive(value: false);
					}
				...
				else
				{
					if (uncheckedRefRW.ValueRW.doorSpineMono.Value != null)
					{
						uncheckedRefRW.ValueRW.doorSpineMono.Value.gameObject.SetActive(value: true);
					}
			...
			if (uncheckedRefRW.ValueRW.openDoorTimer >= uncheckedRefRW.ValueRW.openDoorTime)
			{
				...
				DTool.SetCollider(in uncheckedRefRW2.ValueRO, 33554432u);
				float3 layerPosition2 = DTool.GetLayerPosition(in localTransform.Position, LayerCorrectType.T6Door);
				InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW.ValueRW.ett_Layer).ValueRW.Position = layerPosition2;
				if (uncheckedRefRW.ValueRW.doorSpineMono.Value != null)
				{
					uncheckedRefRW.ValueRW.doorSpineMono.Value.transform.position = localTransform.Position + layerPosition2;
				}
			}
		}
		if (flag)
	...
	private void __AssignQueries(ref SystemState state)
	{
		...
		entityQueryBuilder2 = entityQueryBuilder2.WithAllRW<Door_T6_Dots>();
	...
	{
		__codegen__OnCreate_00005A57_0024BurstDirectCall.Invoke(self, state);
	}
	...
	{
		((Door_T6System*)self.ToPointer())->OnCreate(ref *(SystemState*)state.ToPointer());
	}
}
