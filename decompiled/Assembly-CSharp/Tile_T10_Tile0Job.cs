...
[CompilerGenerated]
[BurstCompile]
public struct Tile_T10_Tile0Job : IJobEntity, IJobChunk
{
	public struct InternalCompilerQueryAndHandleData
	{
		public struct TypeHandle
		{
			...
			[ReadOnly]
			public ComponentTypeHandle<Tile_T10_Tile0> __Tile_T10_Tile0_RO_ComponentTypeHandle;
			...
			public void __AssignHandles(ref SystemState state)
			{
				...
				__Tile_T10_Tile0_RO_ComponentTypeHandle = state.GetComponentTypeHandle<Tile_T10_Tile0>(isReadOnly: true);
			...
			public void Update(ref SystemState state)
			{
				...
				__Tile_T10_Tile0_RO_ComponentTypeHandle.Update(ref state);
		...
		private void __AssignQueries(ref SystemState state)
		{
			...
			EntityQueryBuilder entityQueryBuilder2 = entityQueryBuilder.WithAll<Tile_T10_Tile0>();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Run(ref Tile_T10_Tile0Job job, EntityQuery query)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle Schedule(ref Tile_T10_Tile0Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle ScheduleParallel(ref Tile_T10_Tile0Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void UpdateBaseEntityIndexArray(ref Tile_T10_Tile0Job job, EntityQuery query, ref SystemState state)
		{
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle UpdateBaseEntityIndexArray(ref Tile_T10_Tile0Job job, EntityQuery query, JobHandle dependency, ref SystemState state)
		{
			return dependency;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void AssignEntityManager(ref Tile_T10_Tile0Job job, EntityManager entityManager)
		{
		}
	}
	...
	public void Execute([ChunkIndexInQuery] int chunkIndex, ref LocalTransform localTsf, in Tile_T10_Tile0 tile, in TileBase_Dots tileBase, Entity ett)
	{
		localTsf.Position = tileBase.roomPosition + tileBase.selfPosition.GetFloat3();
		ecb.SetComponentEnabled<Tile_T10_Tile0>(chunkIndex, ett, value: false);
	}
	...
	public void Execute(in ArchetypeChunk chunk, int chunkIndexInQuery, bool useEnabledMask, in v128 chunkEnabledMask)
	{
		...
		if (!useEnabledMask)
		{
			for (int i = 0; i < count; i++)
			{
				...
				Tile_T10_Tile0 tile = default(Tile_T10_Tile0);
				Execute(chunkIndexInQuery, ref localTsf, in tile, in tileBase, ett);
				num++;
			}
			return;
		}
		if (math.countbits(chunkEnabledMask.ULong0 ^ (chunkEnabledMask.ULong0 << 1)) + math.countbits(chunkEnabledMask.ULong1 ^ (chunkEnabledMask.ULong1 << 1)) - 1 <= 4)
		{
			...
			while (InternalCompilerInterface.UnsafeTryGetNextEnabledBitRange(chunkEnabledMask, nextRangeEnd, out nextRangeBegin, out nextRangeEnd))
			{
				while (nextRangeBegin < nextRangeEnd)
				{
					...
					Tile_T10_Tile0 tile = default(Tile_T10_Tile0);
					Execute(chunkIndexInQuery, ref localTsf2, in tile, in tileBase2, ett2);
					nextRangeBegin++;
					num++;
				}
			}
		...
		for (int j = 0; j < num3; j++)
		{
			if ((num2 & 1) != 0L)
			{
				...
				Tile_T10_Tile0 tile = default(Tile_T10_Tile0);
				Execute(chunkIndexInQuery, ref localTsf3, in tile, in tileBase3, ett3);
				num++;
			}
			num2 >>= 1;
		}
		...
		for (int k = 64; k < count; k++)
		{
			if ((num2 & 1) != 0L)
			{
				...
				Tile_T10_Tile0 tile = default(Tile_T10_Tile0);
				Execute(chunkIndexInQuery, ref localTsf4, in tile, in tileBase4, ett4);
				num++;
			}
			num2 >>= 1;
		}
