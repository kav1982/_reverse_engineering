...
[BurstCompile]
[CompilerGenerated]
public struct Tile_T3_Tile1Job : IJobEntity, IJobChunk
{
	public struct InternalCompilerQueryAndHandleData
	{
		...
		{
			[ReadOnly]
			public ComponentTypeHandle<Tile_T3_Tile1_Dots> __Tile_T3_Tile1_Dots_RO_ComponentTypeHandle;
			...
			{
				__Tile_T3_Tile1_Dots_RO_ComponentTypeHandle = state.GetComponentTypeHandle<Tile_T3_Tile1_Dots>(isReadOnly: true);
			...
			{
				__Tile_T3_Tile1_Dots_RO_ComponentTypeHandle.Update(ref state);
		...
		private void __AssignQueries(ref SystemState state)
		{
			...
			EntityQueryBuilder entityQueryBuilder2 = entityQueryBuilder.WithAll<Tile_T3_Tile1_Dots>();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Run(ref Tile_T3_Tile1Job job, EntityQuery query)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle Schedule(ref Tile_T3_Tile1Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle ScheduleParallel(ref Tile_T3_Tile1Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void UpdateBaseEntityIndexArray(ref Tile_T3_Tile1Job job, EntityQuery query, ref SystemState state)
		{
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle UpdateBaseEntityIndexArray(ref Tile_T3_Tile1Job job, EntityQuery query, JobHandle dependency, ref SystemState state)
		{
			return dependency;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void AssignEntityManager(ref Tile_T3_Tile1Job job, EntityManager entityManager)
		{
		}
	}
	...
	public void Execute([ChunkIndexInQuery] int index, in Tile_T3_Tile1_Dots tile, in TileBase_Dots tileBase, ref LocalTransform localTsf, Entity ett)
	{
		...
		if (!flag9)
		{
			ecb.DestroyEntity(index, tile.ett_D);
		}
		if (!flag10)
		{
		...
		if (!flag12)
		{
			ecb.DestroyEntity(index, tile.ett_L);
		}
		if (!flag13)
		{
			ecb.DestroyEntity(index, tile.ett_LR);
		}
		if (!flag14)
		{
		...
		if (!flag16)
		{
			ecb.DestroyEntity(index, tile.ett_Null);
		}
		if (!flag17)
		{
			ecb.DestroyEntity(index, tile.ett_R);
		}
		if (!flag18)
		{
		...
		if (!flag20)
		{
			ecb.DestroyEntity(index, tile.ett_U);
		}
		if (!flag21)
		{
			ecb.DestroyEntity(index, tile.ett_UD);
		}
		if (!flag22)
		{
		...
		if (!flag28)
		{
			ecb.DestroyEntity(index, tile.ett_NoCornerUR);
		}
		if (!flag29)
		{
			ecb.DestroyEntity(index, tile.ett_NoCornerRD);
		}
		if (!flag30)
		{
			ecb.DestroyEntity(index, tile.ett_NoCornerDL);
		}
		if (!flag31)
		{
			ecb.DestroyEntity(index, tile.ett_NoCornerLU);
		}
		ecb.SetComponentEnabled<Tile_T3_Tile1_Dots>(index, ett, value: false);
	}

	[CompilerGenerated]
	public void Execute(in ArchetypeChunk chunk, int chunkIndexInQuery, bool useEnabledMask, in v128 chunkEnabledMask)
	{
		IntPtr nativeArrayPtr = InternalCompilerInterface.UnsafeGetChunkNativeArrayReadOnlyIntPtr(chunk, ref __TypeHandle.__Tile_T3_Tile1_Dots_RO_ComponentTypeHandle);
		...
		if (!useEnabledMask)
		{
			...
			{
				ref Tile_T3_Tile1_Dots tile = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Tile_T3_Tile1_Dots>(nativeArrayPtr, i);
		...
		if (math.countbits(chunkEnabledMask.ULong0 ^ (chunkEnabledMask.ULong0 << 1)) + math.countbits(chunkEnabledMask.ULong1 ^ (chunkEnabledMask.ULong1 << 1)) - 1 <= 4)
		{
			...
			while (InternalCompilerInterface.UnsafeTryGetNextEnabledBitRange(chunkEnabledMask, nextRangeEnd, out nextRangeBegin, out nextRangeEnd))
			{
				...
				{
					ref Tile_T3_Tile1_Dots tile2 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Tile_T3_Tile1_Dots>(nativeArrayPtr, nextRangeBegin);
		...
		for (int j = 0; j < num3; j++)
		{
			if ((num2 & 1) != 0L)
			{
				ref Tile_T3_Tile1_Dots tile3 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Tile_T3_Tile1_Dots>(nativeArrayPtr, j);
		...
		for (int k = 64; k < count; k++)
		{
			if ((num2 & 1) != 0L)
			{
				ref Tile_T3_Tile1_Dots tile4 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Tile_T3_Tile1_Dots>(nativeArrayPtr, k);
