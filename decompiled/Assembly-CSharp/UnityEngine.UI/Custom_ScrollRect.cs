...
[SelectionBase]
public class Custom_ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IEventSystemHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup, ILayoutController
{
	...
	private Coroutine _slidersmooth;

	public int activechildnum;

	public int int_MiddleOffset;

	public int int_widthnum;

	public int int_heightnum;

	private int allRowCount;

	private int currentRow;

	public int scrollNumAtOnce;

	public GameObject content_gameobject;
	...
	{
		if (content_gameobject.transform.childCount != 0)
		{
			ScrollCountActive();
			Update_CurrentRowByValue();
			if (data.scrollDelta.y > 0f)
			{
				ScrollUp(verticalScrollbar, ref currentRow, allRowCount, int_heightnum);
			}
			else if (data.scrollDelta.y < 0f)
			{
				ScrollDown(verticalScrollbar, ref currentRow, allRowCount, int_heightnum);
			}
		}
		else
		{
	...
	public void ScrollUp(Scrollbar scrollbar, ref int currentRow, int allRow, int heightCount)
	{
		StopAllCoroutines();
		_slidersmooth = null;
		currentRow--;
		if ((double)allRow < (double)heightCount * 1.5 && currentRow < heightCount / 2 - 1)
		{
			_slidersmooth = StartCoroutine(Slide_smooth(scrollbar, 1f, scrollbar.size / (float)heightCount));
			return;
		}
		if (currentRow < heightCount / 2 + 1)
		{
			_slidersmooth = StartCoroutine(Slide_smooth(scrollbar, 1f, scrollbar.size / (float)heightCount));
			currentRow = heightCount / 2 + 1;
			return;
		}
		if (currentRow > allRow - heightCount / 2 + 1)
		{
			currentRow = allRow - heightCount / 2 + 1;
			_slidersmooth = StartCoroutine(Slide_smooth(scrollbar, 0f, scrollbar.size / (float)heightCount));
		}
		_slidersmooth = StartCoroutine(Slide_smooth(scrollbar, 1f - (float)(currentRow - heightCount / 2 - 1) / ((float)allRow - (float)heightCount), scrollbar.size / (float)heightCount));
	}

	public void ScrollDown(Scrollbar scrollbar, ref int currentRow, int allRow, int heightCount)
	{
		StopAllCoroutines();
		_slidersmooth = null;
		currentRow++;
		if ((double)allRow < (double)heightCount * 1.5 && allRowCount - currentRow - heightCount / 2 < 0)
		{
			_slidersmooth = StartCoroutine(Slide_smooth(scrollbar, 0f, scrollbar.size / (float)heightCount));
			return;
		}
		...
		else if (currentRow >= allRow - heightCount / 2 + 1)
		{
			currentRow = allRow - heightCount / 2 + 1;
			_slidersmooth = StartCoroutine(Slide_smooth(scrollbar, 0f, scrollbar.size / (float)heightCount));
			return;
	...
	public int ClampCurrentRow(int currentrow, bool slideDirectionDown)
	{
		if (int_heightnum % 2 == 0)
		{
			if (currentrow <= int_heightnum / 2 - int_MiddleOffset)
			{
				return int_heightnum / 2 - int_MiddleOffset;
			}
			if (currentrow > allRowCount - int_heightnum / 2 + int_MiddleOffset)
			{
				return allRowCount - int_heightnum / 2 + 1 + int_MiddleOffset;
			}
			if (!slideDirectionDown)
			{
				if (currentrow <= currentRow - int_MiddleOffset)
				{
					return currentrow;
				}
			}
			else if (currentrow >= currentRow + int_MiddleOffset)
			{
				return currentrow;
			}
		}
		else
		{
			...
			if (currentrow >= allRowCount - int_heightnum / 2 + int_MiddleOffset)
			{
				return allRowCount - int_heightnum / 2 + int_MiddleOffset;
			}
			if (!slideDirectionDown)
			{
				if (int_MiddleOffset == 0)
				{
					return currentrow;
				}
				if (currentrow <= currentRow - int_MiddleOffset)
	...
	public void ScrollUpdate(int _currentRow, bool slideDirection = true)
	{
		ScrollCountActive();
		_currentRow = IndexToActiveIndex(_currentRow);
		_currentRow = ClampCurrentRow(_currentRow, slideDirection);
		if (_currentRow == -1)
		{
			return;
		}
		...
		if (int_heightnum % 2 == 0)
		{
			if (_currentRow <= int_heightnum / 2 - int_MiddleOffset)
			{
				StartCoroutine(Slide_smooth(verticalScrollbar, 1f, verticalScrollbar.size / (float)int_heightnum));
			}
			else if (_currentRow >= allRowCount - (int_heightnum / 2 - 1) + int_MiddleOffset)
			{
				StartCoroutine(Slide_smooth(verticalScrollbar, 0f, verticalScrollbar.size / (float)int_heightnum));
			}
			else if (slideDirection)
			{
				StartCoroutine(Slide_smooth(verticalScrollbar, 1f - (float)(_currentRow - (int_heightnum / 2 + 1 - int_MiddleOffset)) / ((float)allRowCount - (float)int_heightnum), verticalScrollbar.size / (float)int_heightnum));
			}
			else
			{
				StartCoroutine(Slide_smooth(verticalScrollbar, 1f - (float)(_currentRow - (int_heightnum / 2 + int_MiddleOffset)) / ((float)allRowCount - (float)int_heightnum), verticalScrollbar.size / (float)int_heightnum));
			}
		}
		else if (slideDirection)
		{
			if (_currentRow < int_heightnum / 2 + 1 + int_MiddleOffset)
			{
				StartCoroutine(Slide_smooth(verticalScrollbar, 1f, verticalScrollbar.size / (float)int_heightnum));
			}
			else if (_currentRow > allRowCount - (int_heightnum / 2 + 1 - int_MiddleOffset))
		...
		else if (_currentRow < int_heightnum / 2 + 1 - int_MiddleOffset)
		{
			StartCoroutine(Slide_smooth(verticalScrollbar, 1f, verticalScrollbar.size / (float)int_heightnum));
		}
		else if (_currentRow > allRowCount - (int_heightnum / 2 + 1 + int_MiddleOffset))
	...
	public int IndexToActiveIndex(int index)
	{
		int num = 0;
		for (int i = 0; i < content_gameobject.transform.childCount; i++)
		{
			if (i == index)
			{
				return num;
			}
			if (content_gameobject.transform.GetChild(i).gameObject.activeSelf)
			{
				num++;
			}
		}
	...
	public IEnumerator Slide_smooth(Scrollbar scrollbar, float value, float speed)
	{
		int num = (int)MathF.Round((float)(allRowCount - int_heightnum) * (1f - value));
		currentRow = num + int_heightnum / 2 + 1;
		...
		if (scrollbar.value > value)
		{
			while (scrollbar.value > value + speed)
			{
	...
	public void ScrollCountActive()
	{
		...
		if (activechildnum != 0)
		{
			allRowCount = Mathf.CeilToInt((float)activechildnum / (float)int_widthnum);
		}
	}

	public void Update_CurrentRowByValue()
	{
		int num = (int)MathF.Round((float)(allRowCount - int_heightnum) * (1f - verticalScrollbar.value));
		currentRow = num + int_heightnum / 2 + 1;
	}

	public int CurrentRowmiddle()
	{
		return (int)MathF.Round((float)(allRowCount - int_heightnum) * (1f - verticalScrollbar.value)) + int_heightnum / 2 + 1;
	}

	protected Custom_ScrollRect()
	{
	}
	...
	{
		if (!m_Dragging || eventData.button != 0 || !IsActive() || !RectTransformUtility.ScreenPointToLocalPointInRectangle(viewRect, eventData.position, eventData.pressEventCamera, out var localPoint))
		{
			return;
		}
		UpdateBounds();
