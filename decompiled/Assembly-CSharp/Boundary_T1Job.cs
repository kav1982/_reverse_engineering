...
[CompilerGenerated]
[BurstCompile]
public struct Boundary_T1Job : IJobEntity, IJobChunk
{
	public struct InternalCompilerQueryAndHandleData
	{
		public struct TypeHandle
		{
			public ComponentTypeHandle<Boundary_T1_Dots> __Boundary_T1_Dots_RW_ComponentTypeHandle;
			...
			{
				__Boundary_T1_Dots_RW_ComponentTypeHandle = state.GetComponentTypeHandle<Boundary_T1_Dots>();
			...
			{
				__Boundary_T1_Dots_RW_ComponentTypeHandle.Update(ref state);
		...
		private void __AssignQueries(ref SystemState state)
		{
			...
			entityQueryBuilder2 = entityQueryBuilder2.WithAllRW<Boundary_T1_Dots>();
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Run(ref Boundary_T1Job job, EntityQuery query)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle Schedule(ref Boundary_T1Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle ScheduleParallel(ref Boundary_T1Job job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void UpdateBaseEntityIndexArray(ref Boundary_T1Job job, EntityQuery query, ref SystemState state)
		{
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle UpdateBaseEntityIndexArray(ref Boundary_T1Job job, EntityQuery query, JobHandle dependency, ref SystemState state)
		{
			return dependency;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void AssignEntityManager(ref Boundary_T1Job job, EntityManager entityManager)
		{
		}
	}
	...
	public void Execute([ChunkIndexInQuery] int index, ref Boundary_T1_Dots boundary, in BoundaryBase_Dots boundaryBase, Entity ett)
	{
		...
		refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerLava);
		refRW2.ValueRW.Position = DTool.GetLayerPosition(in refRW.ValueRW.Position, LayerCorrectType.Lava1);
		refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerLava2);
		...
		refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
		...
		if (flag && flag3 && !flag5 && !flag7)
		{
			...
			flag22 = true;
			refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
			refRW2.ValueRW.Position += new float3(0f - boundary.railOffset, 0f - boundary.railOffset, 0f);
			flag29 = true;
		}
		else if (!flag && flag3 && flag5 && !flag7)
		{
			...
			flag14 = true;
			refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerLava);
			refRW2.ValueRW.Position = layerPosition + boundary.lavaOffset;
			flag21 = true;
			refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
			refRW2.ValueRW.Position += new float3(0f - boundary.railOffset, boundary.railOffset, 0f);
			flag27 = true;
		}
		else if (!flag && !flag3 && flag5 && flag7)
		{
		...
		else if (flag && flag3 && !flag5 && flag7)
		{
			flag20 = true;
			refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
			refRW2.ValueRW.Position += new float3(0f, 0f - boundary.railOffset, 0f);
			flag26 = true;
		}
		else if (flag && flag3 && flag5 && !flag7)
		{
			...
			flag23 = true;
			refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
			refRW2.ValueRW.Position += new float3(0f - boundary.railOffset, 0f, 0f);
			flag30 = true;
		}
		else if (!flag && flag3 && flag5 && flag7)
		{
			...
			flag20 = true;
			refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
			refRW2.ValueRW.Position += new float3(0f, boundary.railOffset, 0f);
			flag28 = true;
		}
		else if (flag && !flag3 && flag5 && flag7)
		{
		...
		else if (flag && flag3 && flag5 && flag7)
		{
			if (!flag2)
			{
				...
				flag19 = true;
				refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
				refRW2.ValueRW.Position += new float3(boundary.railOffset, boundary.railOffset, 0f);
				flag25 = true;
			}
			else if (!flag4)
			{
				...
				flag18 = true;
				refRW2 = cluLocalTsf.GetRefRW(boundary.ett_LayerRail);
				refRW2.ValueRW.Position += new float3(boundary.railOffset, 0f - boundary.railOffset, -0.0001f);
				flag24 = true;
			}
			else if (!flag6)
			{
		...
		if (!flag18)
		{
			ecb.DestroyEntity(index, boundary.ett_Rail_Corner_RD);
		}
		if (!flag19)
		{
			ecb.DestroyEntity(index, boundary.ett_Rail_Corner_UR);
		}
		if (!flag20)
		{
			ecb.DestroyEntity(index, boundary.ett_Rail_LUR);
		}
		if (!flag21)
		{
			ecb.DestroyEntity(index, boundary.ett_Rail_RD);
		}
		if (!flag22)
		{
			ecb.DestroyEntity(index, boundary.ett_Rail_UR);
		}
		if (!flag23)
		{
			ecb.DestroyEntity(index, boundary.ett_Rail_URD);
		}
		if (!flag24)
		{
		...
		ecb.SetComponentEnabled<Boundary_T1_Dots>(index, ett, value: false);
	}

	[CompilerGenerated]
	public void Execute(in ArchetypeChunk chunk, int chunkIndexInQuery, bool useEnabledMask, in v128 chunkEnabledMask)
	{
		IntPtr nativeArrayPtr = InternalCompilerInterface.UnsafeGetChunkNativeArrayIntPtr(chunk, ref __TypeHandle.__Boundary_T1_Dots_RW_ComponentTypeHandle);
		...
		if (!useEnabledMask)
		{
			...
			{
				ref Boundary_T1_Dots boundary = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T1_Dots>(nativeArrayPtr, i);
		...
		if (math.countbits(chunkEnabledMask.ULong0 ^ (chunkEnabledMask.ULong0 << 1)) + math.countbits(chunkEnabledMask.ULong1 ^ (chunkEnabledMask.ULong1 << 1)) - 1 <= 4)
		{
			...
			while (InternalCompilerInterface.UnsafeTryGetNextEnabledBitRange(chunkEnabledMask, nextRangeEnd, out nextRangeBegin, out nextRangeEnd))
			{
				...
				{
					ref Boundary_T1_Dots boundary2 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T1_Dots>(nativeArrayPtr, nextRangeBegin);
		...
		for (int j = 0; j < num3; j++)
		{
			if ((num2 & 1) != 0L)
			{
				ref Boundary_T1_Dots boundary3 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T1_Dots>(nativeArrayPtr, j);
		...
		for (int k = 64; k < count; k++)
		{
			if ((num2 & 1) != 0L)
			{
				ref Boundary_T1_Dots boundary4 = ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Boundary_T1_Dots>(nativeArrayPtr, k);
