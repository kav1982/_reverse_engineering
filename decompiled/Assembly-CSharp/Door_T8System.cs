...
[CompilerGenerated]
[BurstCompile]
public struct Door_T8System : ISystem, ISystemCompilerGenerated
{
	[StructLayout(LayoutKind.Sequential, Size = 1)]
	private readonly struct IFE_1946954582_0
	{
		...
		public struct TypeHandle
		{
			...
			{
				item1_ComponentTypeHandle_RW = systemState.GetComponentTypeHandle<Door_T8_Dots>();
		...
		{
			state.EntityManager.CompleteDependencyBeforeRW<Door_T8_Dots>();
	...
	{
		public IFE_1946954582_0.TypeHandle __IFE_1946954582_0_TypeHandle;

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void __AssignHandles(ref SystemState state)
		{
			__IFE_1946954582_0_TypeHandle = new IFE_1946954582_0.TypeHandle(ref state);
		}
	}

	[UnmanagedFunctionPointer(CallingConvention.Cdecl)]
	internal delegate void __codegen__OnCreate_00005A6F_0024PostfixBurstDelegate(IntPtr self, IntPtr state);

	internal static class __codegen__OnCreate_00005A6F_0024BurstDirectCall
	{
		...
		private static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = BurstCompiler.CompileFunctionPointer<__codegen__OnCreate_00005A6F_0024PostfixBurstDelegate>(delegate(IntPtr self, IntPtr state)
				{
					Invoke(self, state);
				}).Value;
			}
	...
	private EntityQuery __query_1946954582_0;

	private EntityQuery __query_1946954582_1;

	[BurstCompile]
	public void OnCreate(ref SystemState state)
	{
		state.RequireForUpdate<EndSimulationEntityCommandBufferSystem.Singleton>();
		state.RequireForUpdate<Door_T8_Dots>();
	}
	...
	public void OnUpdate(ref SystemState state)
	{
		...
		foreach (QueryEnumerableWithEntity<InternalCompilerInterface.UncheckedRefRW<Door_T8_Dots>, InternalCompilerInterface.UncheckedRefRW<PhysicsCollider>, InternalCompilerInterface.UncheckedRefRW<DoorBase_Dots>, LocalTransform, InternalCompilerInterface.UncheckedRefRW<InteractiveObj_Dots>> item6 in IFE_1946954582_0.Query(__query_1946954582_0, __TypeHandle.__IFE_1946954582_0_TypeHandle, ref state))
		{
			item6.Deconstruct(out var item, out var item2, out var item3, out var item4, out var item5, out var entity);
			InternalCompilerInterface.UncheckedRefRW<Door_T8_Dots> uncheckedRefRW = item;
			...
			if (!uncheckedRefRW.ValueRW.isInitialized)
			{
				...
				EntityCommandBuffer ecb = __query_1946954582_1.GetSingleton<EndSimulationEntityCommandBufferSystem.Singleton>().CreateCommandBuffer(state.WorldUnmanaged);
				uncheckedRefRW2.ValueRW.MakeUnique(in entity2, ecb);
				switch (uncheckedRefRW.ValueRW.ThemeType)
				{
				default:
					uncheckedRefRW.ValueRW.doorT8Mono.Value = ObjPoolMgr.Inst.GetGO("Prefabs/Mixed/Door_T7Mono", localTransform.Position).GetComponent<Door_T8Mono>();
					break;
				case RoomThemeType.Theme8_Chapter4:
					uncheckedRefRW.ValueRW.doorT8Mono.Value = ObjPoolMgr.Inst.GetGO("Prefabs/Mixed/Door_T8Mono", localTransform.Position).GetComponent<Door_T8Mono>();
					break;
				case RoomThemeType.Theme9_Chapter4_2:
					uncheckedRefRW.ValueRW.doorT8Mono.Value = ObjPoolMgr.Inst.GetGO("Prefabs/Mixed/Door_T9Mono", localTransform.Position).GetComponent<Door_T8Mono>();
					break;
				}
				uncheckedRefRW.ValueRW.doorT8Mono.Value.UpdateDisplay(uncheckedRefRW3.ValueRW.rewardType);
			}
			if (uncheckedRefRW4.ValueRW.onSelect)
			{
				uncheckedRefRW4.ValueRW.onSelect = false;
				uncheckedRefRW.ValueRW.doorT8Mono.Value.go_Outline.SetActive(value: true);
			}
			if (uncheckedRefRW4.ValueRW.onDeselect)
			{
				uncheckedRefRW4.ValueRW.onDeselect = false;
				uncheckedRefRW.ValueRW.doorT8Mono.Value.go_Outline.SetActive(value: false);
			}
			...
			if (uncheckedRefRW3.ValueRW.onOpen)
			{
				...
				if (LevelMgr.Inst.CurrentRoomCtrller.AllLevelRewardPicked && uncheckedRefRW3.ValueRW.rewardType != LevelRewardType.Ruined)
				{
					...
					uncheckedRefRW.ValueRW.doorT8Mono.Value.Open();
			...
			if (uncheckedRefRW3.ValueRW.onOpenDirect)
			{
				...
				if (LevelMgr.Inst.CurrentRoomCtrller.AllLevelRewardPicked && uncheckedRefRW3.ValueRW.rewardType != LevelRewardType.Ruined)
				{
					uncheckedRefRW.ValueRW.doorT8Mono.Value.OpenDirect();
	...
	private void __AssignQueries(ref SystemState state)
	{
		...
		entityQueryBuilder2 = entityQueryBuilder2.WithAllRW<Door_T8_Dots>();
	...
	{
		__codegen__OnCreate_00005A6F_0024BurstDirectCall.Invoke(self, state);
	}
	...
	{
		((Door_T8System*)self.ToPointer())->OnCreate(ref *(SystemState*)state.ToPointer());
	}
}
