...
public class Boss56E55HexShooter : MonoBehaviour
{
	...
	private UnitBase.UnitSpellModifier usm;
	...
	private void OnEnable()
	{
		...
		usm.Shooter = shooterEntity;
		usm.ApplyToSSP(ref ssp);
		ssp.DisableResize = true;
	}

	public void InitialData(Entity shooterEntity, Vector3 targetPosition)
	{
	...
	public void CastHexSkill()
	{
		currentSkill = Elite55Skill.HexAttack;
		EnterState(Elite55State.TeleportEndCastSkill);
	}

	public void CastCatchSkill()
	{
			...
			case Elite55Skill.HexAttack:
			{
				if (!isSkillInitialize)
				{
					...
					for (int num7 = 0; num7 < 6; num7++)
					{
						...
						component3.ShootByOtherSource(shooterEntity);
					}
				}
	...
	private void EnterState(Elite55State newState)
	{
		state = newState;
		switch (state)
		{
		...
		case Elite55State.Teleport:
			teleportTimer = 0f;
			teleportPosition = Tool2D.GetNavMeshPointIngoreZ(targetPoint + UnityEngine.Random.insideUnitSphere.IgnoreZ().normalized * UnityEngine.Random.Range(TeleportRange.x, TeleportRange.y)) + new Vector3(0f, 1.1f, 0f);
