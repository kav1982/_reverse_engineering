...
public class ETFXProjectileScript : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		{
			base.transform.rotation = Quaternion.LookRotation(GetComponent<Rigidbody>().linearVelocity);
		}
		float radius = ((!base.transform.GetComponent<SphereCollider>()) ? colliderRadius : base.transform.GetComponent<SphereCollider>().radius);
		Vector3 linearVelocity = base.transform.GetComponent<Rigidbody>().linearVelocity;
		...
		float maxDistance = base.transform.GetComponent<Rigidbody>().linearVelocity.magnitude * Time.deltaTime;
		if (!Physics.SphereCast(base.transform.position, radius, linearVelocity, out var hitInfo, maxDistance))
		{
			return;
		}
