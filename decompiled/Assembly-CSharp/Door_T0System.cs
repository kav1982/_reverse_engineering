...
public struct Door_T0System : ISystem, ISystemCompilerGenerated
{
	[StructLayout(LayoutKind.Sequential, Size = 1)]
	private readonly struct IFE_1938041679_0
	{
		...
		public struct TypeHandle
		{
			...
			{
				item1_ComponentTypeHandle_RW = systemState.GetComponentTypeHandle<Door_T0_Dots>();
		...
		{
			state.EntityManager.CompleteDependencyBeforeRW<Door_T0_Dots>();
	...
	{
		public IFE_1938041679_0.TypeHandle __IFE_1938041679_0_TypeHandle;
		...
		{
			__IFE_1938041679_0_TypeHandle = new IFE_1938041679_0.TypeHandle(ref state);
	...
	internal delegate void __codegen__OnCreate_00005A0F_0024PostfixBurstDelegate(IntPtr self, IntPtr state);

	internal static class __codegen__OnCreate_00005A0F_0024BurstDirectCall
	{
		...
		private static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = BurstCompiler.CompileFunctionPointer<__codegen__OnCreate_00005A0F_0024PostfixBurstDelegate>(delegate(IntPtr self, IntPtr state)
				{
					Invoke(self, state);
				}).Value;
			}
	...
	private EntityQuery __query_1938041679_0;

	private EntityQuery __query_1938041679_1;

	[BurstCompile]
	public void OnCreate(ref SystemState state)
	{
		state.RequireForUpdate<EndSimulationEntityCommandBufferSystem.Singleton>();
		state.RequireForUpdate<Door_T0_Dots>();
	}
	...
	public void OnUpdate(ref SystemState state)
	{
		...
		foreach (QueryEnumerableWithEntity<InternalCompilerInterface.UncheckedRefRW<Door_T0_Dots>, InternalCompilerInterface.UncheckedRefRW<PhysicsCollider>, InternalCompilerInterface.UncheckedRefRW<DoorBase_Dots>, LocalTransform, InternalCompilerInterface.UncheckedRefRW<InteractiveObj_Dots>> item6 in IFE_1938041679_0.Query(__query_1938041679_0, __TypeHandle.__IFE_1938041679_0_TypeHandle, ref state))
		{
			item6.Deconstruct(out var item, out var item2, out var item3, out var item4, out var item5, out var entity);
			InternalCompilerInterface.UncheckedRefRW<Door_T0_Dots> uncheckedRefRW = item;
			...
			if (!uncheckedRefRW.ValueRW.isInitialized)
			{
				...
				EntityCommandBuffer ecb = __query_1938041679_1.GetSingleton<EndSimulationEntityCommandBufferSystem.Singleton>().CreateCommandBuffer(state.WorldUnmanaged);
			...
			if (uncheckedRefRW3.ValueRW.onUpdateDisplay)
			{
				...
				if (uncheckedRefRW3.ValueRW.rewardType == LevelRewardType.Ruined)
				{
					InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_Portal).ValueRW.Scale = 0f;
					InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_Door).ValueRW.Scale = 0f;
					InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_DoorRuined).ValueRW.Scale = 1f;
				}
				else
				{
					InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_Door).ValueRW.Scale = 1f;
					InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_DoorRuined).ValueRW.Scale = 0f;
			...
			if (uncheckedRefRW3.ValueRW.onOpenDirect)
			{
				...
				if (LevelMgr.Inst.CurrentRoomCtrller.AllLevelRewardPicked && uncheckedRefRW3.ValueRW.rewardType != LevelRewardType.Ruined)
				{
					InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__MatOverrideOffset_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_Door2).ValueRW.offset = new float2(0f, uncheckedRefRW3.ValueRW.openDoorFinalOffsetY);
					DTool.SetCollider(in uncheckedRefRW2.ValueRO, 33554432u);
				}
			}
			if (uncheckedRefRW3.ValueRW.isOpening)
			{
				uncheckedRefRW3.ValueRW.doorCurrentOffsetY = DTool.MoveTowards(uncheckedRefRW3.ValueRW.doorCurrentOffsetY, uncheckedRefRW3.ValueRW.openDoorFinalOffsetY, uncheckedRefRW3.ValueRW.openDoorSpeed * state.WorldUnmanaged.Time.DeltaTime);
				if (uncheckedRefRW3.ValueRW.doorCurrentOffsetY == uncheckedRefRW3.ValueRW.openDoorFinalOffsetY)
				{
					uncheckedRefRW3.ValueRW.isOpening = false;
				}
				InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__MatOverrideOffset_RW_ComponentLookup, ref state, uncheckedRefRW3.ValueRW.ett_Door2).ValueRW.offset = new float2(0f, uncheckedRefRW3.ValueRW.doorCurrentOffsetY);
				DTool.SetCollider(in uncheckedRefRW2.ValueRO, 33554432u);
			}
		}
		if (flag)
	...
	private void __AssignQueries(ref SystemState state)
	{
		...
		entityQueryBuilder2 = entityQueryBuilder2.WithAllRW<Door_T0_Dots>();
	...
	{
		__codegen__OnCreate_00005A0F_0024BurstDirectCall.Invoke(self, state);
	}
	...
	{
		((Door_T0System*)self.ToPointer())->OnCreate(ref *(SystemState*)state.ToPointer());
	}
}
