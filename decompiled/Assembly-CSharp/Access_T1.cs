...
public class Access_T1 : AccessBase
{
	[Space(50f)]
	...
	public GameObject pfb_WallLight;
	...
	private void AnimaEvent(string eventStr)
	{
		if (!(eventStr == "OpenFinish"))
		{
			if (!(eventStr == "CloseFinish"))
			{
	...
	public override void Initialize(RoomController roomCtrller, AccessType type, FourDir dir)
	{
		...
		animaEvent.DoAction = AnimaEvent;
		...
		case FourDir.Right:
			...
			if (go_LightR != null && roomCtrller.roomCfg.type != RoomType.Boss && roomCtrller.roomCfg.type != RoomType.BloodRelic)
			{
				go_LightR.SetActive(value: true);
			}
			if (roomCtrller.roomCfg.type != RoomType.Boss && roomCtrller.roomCfg.themeType != RoomThemeType.Theme1_Chapter2_Cliff && roomCtrller.roomCfg.themeType != RoomThemeType.Theme5_Chapter1_Water && roomCtrller.roomCfg.themeType != RoomThemeType.Theme19_Chapter2_Shortcut2 && roomCtrller.roomCfg.type != RoomType.BloodRelic)
			{
				pfb_WallLight = Object.Instantiate(pfb_WallLight, base.transform.position + LightPoints[0], Quaternion.identity, base.transform.parent);
				pfb_WallLight.transform.localScale = new Vector3(-1f, 1f, 1f);
				pfb_WallLight = Object.Instantiate(pfb_WallLight, base.transform.position + LightPoints[1], Quaternion.identity, base.transform.parent);
				pfb_WallLight.transform.localScale = new Vector3(-1f, 1f, 1f);
			}
			break;
		case FourDir.Down:
		...
		case FourDir.Left:
			...
			if (roomCtrller.roomCfg.type != RoomType.Boss && roomCtrller.roomCfg.themeType != RoomThemeType.Theme1_Chapter2_Cliff && roomCtrller.roomCfg.themeType != RoomThemeType.Theme5_Chapter1_Water && roomCtrller.roomCfg.themeType != RoomThemeType.Theme19_Chapter2_Shortcut2 && roomCtrller.roomCfg.type != RoomType.BloodRelic)
			{
				Object.Instantiate(pfb_WallLight, base.transform.position + LightPoints[2], Quaternion.identity, base.transform.parent);
				Object.Instantiate(pfb_WallLight, base.transform.position + LightPoints[3], Quaternion.identity, base.transform.parent);
			}
			break;
		default:
			Debug.LogError(dir);
