...
public class Elite11_ChildTentacle : MonoBehaviour
{
	[Serializable]
	public class Node
	{
	...
	private void Update()
	{
		...
		for (int i = 0; i < nodeCount; i++)
		{
			...
			case 1:
			{
				...
				allNode[i - 1].transform.localEulerAngles = new Vector3(0f, 0f, Tool2D.IgnoreZAngleWithSign(base.transform.parent.right, allNode[i].tempPosition + vector2 - allNode[i - 1].transform.position));
			...
			default:
			{
				...
				allNode[i - 1].transform.localEulerAngles = new Vector3(0f, 0f, Tool2D.IgnoreZAngleWithSign(allNode[i - 2].transform.right, allNode[i].tempPosition + vector - allNode[i - 1].transform.position));
