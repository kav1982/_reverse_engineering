...
[CompilerGenerated]
[BurstCompile]
public struct Spell1025NormalEffectUpdateJob : IJobEntity, IJobChunk
{
	public struct InternalCompilerQueryAndHandleData
	{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void Run(ref Spell1025NormalEffectUpdateJob job, EntityQuery query)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle Schedule(ref Spell1025NormalEffectUpdateJob job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle ScheduleParallel(ref Spell1025NormalEffectUpdateJob job, EntityQuery query, JobHandle dependency)
		{
			job.__TypeHandle = __TypeHandle;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void UpdateBaseEntityIndexArray(ref Spell1025NormalEffectUpdateJob job, EntityQuery query, ref SystemState state)
		{
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public JobHandle UpdateBaseEntityIndexArray(ref Spell1025NormalEffectUpdateJob job, EntityQuery query, JobHandle dependency, ref SystemState state)
		{
			return dependency;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void AssignEntityManager(ref Spell1025NormalEffectUpdateJob job, EntityManager entityManager)
		{
		}
	}
	...
	[BurstCompile]
	private void Execute([ChunkIndexInQuery] int index, in SpellMovementComponentData movement, ref Spell1025DragonBreathData spell, in LocalTransform transform, in SpellConfigComponentData config, in SpellComponentData data)
	{
		if (movement.Type == SpellSpecialMovementType.Rotation)
		{
			return;
		}
		spell.GlobalParticleEmitTimer -= DeltaTime;
		if (spell.GlobalParticleEmitTimer <= 0f)
		{
			float num = 10f;
			float num2 = spell.currentAttackDistance * 2.6f;
			float3 position = DTool.CalculateLayerPosition(in transform.Position, LayerCorrectType.Coordinate);
			float scatter = config.Scatter;
			for (float num3 = scatter * -0.5f + num; num3 <= scatter * 0.5f - num; num3 += num)
			{
				float3 @float = DTool.RotateDir(movement.Direction, num3 + Random.NextFloatByChunkIndex(index, num * -0.5f, num3 * 0.5f));
				ref EntityCommandBuffer.ParallelWriter reference = ref cmd;
				FixedString32Bytes effectName = "Fire";
				float scale = 1f;
				ref SpellSingleton spellSingleton = ref SpellSingleton;
				float3 velocity = @float * num2;
				reference.CreateSpellGlobalParticle(0, in effectName, in position, in scale, in config, in data, in spellSingleton, in velocity);
			}
			spell.GlobalParticleEmitTimer = 0.04f;
		}
	}
	...
	public void Execute(in ArchetypeChunk chunk, int chunkIndexInQuery, bool useEnabledMask, in v128 chunkEnabledMask)
	{
		...
		IntPtr nativeArrayPtr2 = InternalCompilerInterface.UnsafeGetChunkNativeArrayIntPtr(chunk, ref __TypeHandle.__Spell1025DragonBreathData_RW_ComponentTypeHandle);
		...
		if (!useEnabledMask)
		{
			...
			{
				Execute(chunkIndexInQuery, in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellMovementComponentData>(nativeArrayPtr, i), ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Spell1025DragonBreathData>(nativeArrayPtr2, i), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<LocalTransform>(nativeArrayPtr3, i), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellConfigComponentData>(nativeArrayPtr4, i), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellComponentData>(nativeArrayPtr5, i));
				num++;
			}
			return;
		}
		...
		for (int j = 0; j < num3; j++)
		{
			if ((num2 & 1) != 0L)
			{
				Execute(chunkIndexInQuery, in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellMovementComponentData>(nativeArrayPtr, j), ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Spell1025DragonBreathData>(nativeArrayPtr2, j), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<LocalTransform>(nativeArrayPtr3, j), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellConfigComponentData>(nativeArrayPtr4, j), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellComponentData>(nativeArrayPtr5, j));
				num++;
			}
			num2 >>= 1;
		}
		...
		for (int k = 64; k < count; k++)
		{
			if ((num2 & 1) != 0L)
			{
				Execute(chunkIndexInQuery, in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellMovementComponentData>(nativeArrayPtr, k), ref InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<Spell1025DragonBreathData>(nativeArrayPtr2, k), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<LocalTransform>(nativeArrayPtr3, k), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellConfigComponentData>(nativeArrayPtr4, k), in InternalCompilerInterface.UnsafeGetRefToNativeArrayPtrElement<SpellComponentData>(nativeArrayPtr5, k));
				num++;
			}
			num2 >>= 1;
		}
