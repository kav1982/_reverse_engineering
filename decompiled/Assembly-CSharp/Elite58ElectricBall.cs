...
public class Elite58ElectricBall : MonoBehaviour
{
	...
	public void InitialData(Vector3 moveDirection, float duration, float speed, float explosionDamage, float explosionRange, float explosionExistTime, float explosionDelayEffect, float moveSpeedRatio, float bulletSpeedRatio)
	{
	...
	private void Update()
	{
		...
		else
		{
			...
			Vector3 normalized = (PlayerMgr.Inst.PlayerPoint - base.transform.position).IgnoreZ().normalized;
			moveDirection = Vector3.Lerp(moveDirection, normalized, DirectionLerpSpeed * Time.deltaTime);
			...
			if (flag || moveTimer >= moveDuration || !Tool2D.PointOnNavMesh(base.transform.position.IgnoreZ()))
			{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			case 2097152u:
				if (UnitDotsSyncSystem.HasComponent<UnitProperty_Dots>(distanceHitResult.entity))
				{
					...
					info.knockbackForce = -Tool2D.IgnoreZV2ToV1Normal(distanceHitResult.point, base.transform.position) * 5f * ExplosionPullForce;
