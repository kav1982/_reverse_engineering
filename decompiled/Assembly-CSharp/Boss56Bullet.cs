...
public class Boss56Bullet : MonoBehaviour, IDotsTriggerReceiver, IDotsPhysicsReciever
{
	[Header("表现")]
	...
	public void Initialize(Vector3 direction, float speed, Entity master)
	{
		...
		EventMgr.EndlessStageClear = (Action)Delegate.Combine(EventMgr.EndlessStageClear, new Action(OnEndlessStageClear));
	}

	private void OnEndlessStageClear()
	{
		recycle = true;
	}
	...
	{
		EventMgr.EndlessStageClear = (Action)Delegate.Remove(EventMgr.EndlessStageClear, new Action(OnEndlessStageClear));
	}
	...
	private void Update()
	{
		...
		if (Tool2D.PointWithinRange(vector, roomCenterPoint, roomWidth + 6f, roomHeight + 6f) != vector)
		{
		...
		bulletHead.transform.localEulerAngles = new Vector3(0f, 0f, Tool2D.IgnoreZAngleWithSign(Vector3.right, direction));
		if (!recycle)
		{
			return;
		}
