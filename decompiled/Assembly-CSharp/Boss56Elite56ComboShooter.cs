...
public class Boss56Elite56ComboShooter : MonoBehaviour
{
	...
	public void InitialGiantBombData(Entity shooterEntity, Vector3 targetPoint, float initialDelay)
	{
	...
	public void InitialSplitMissileData(Entity shooterEntity, float duration, float rotateSpeed, float maxRotateAngle, Vector3 baseShootDirection, float shootInterval)
	{
		...
		SM_RotateSpeed = rotateSpeed;
		SM_MaxRotateAngle = maxRotateAngle;
		SM_ShootInterval = shootInterval;
		SM_BaseDirection = baseShootDirection;
		SM_ShootDirection = baseShootDirection;
	}
	...
	private void UpdateState()
	{
		...
		case Elite56SkillCannon.HMissile:
			if (!(skillTimer >= SM_Duration))
			{
				SM_RotateTimer += Time.deltaTime;
				SM_ShootDirection = GetCurrentShiftDirection();
				UpdateCannonState();
				HMissileShootTimer += Time.deltaTime;
				if (HMissileShootTimer >= SM_ShootInterval)
				{
					HMissileShootTimer -= SM_ShootInterval;
		...
		{
			if (isVMissileLaunched || skillTimer < shootGiantBombWaitTime)
			{
				break;
			}
			if (!isLockStart)
	...
	{
		if (currentSkill == Elite56SkillCannon.HMissile)
		{
			...
			bool num2 = Vector3.Angle(vector - ShootPointTransform.position, CannonTransform.right) <= 5f;
			float a = Tool2D.IgnoreZDistance(PlayerMgr.Inst.PlayerPoint + new Vector3(0f, 0.3f, 0f), ShootPointTransform.position);
			a = ((!num2) ? HMissileLaserDistance : Mathf.Min(a, HMissileLaserDistance));
