...
public struct Access_T8System : ISystem, ISystemCompilerGenerated
{
	[StructLayout(LayoutKind.Sequential, Size = 1)]
	private readonly struct IFE_1943066277_0
	{
		...
		public struct TypeHandle
		{
			...
			{
				item1_ComponentTypeHandle_RW = systemState.GetComponentTypeHandle<Access_T8_Dots>();
		...
		{
			state.EntityManager.CompleteDependencyBeforeRW<Access_T8_Dots>();
	...
	{
		public IFE_1943066277_0.TypeHandle __IFE_1943066277_0_TypeHandle;
		...
		{
			__IFE_1943066277_0_TypeHandle = new IFE_1943066277_0.TypeHandle(ref state);
	...
	internal delegate void __codegen__OnCreate_000057C6_0024PostfixBurstDelegate(IntPtr self, IntPtr state);

	internal static class __codegen__OnCreate_000057C6_0024BurstDirectCall
	{
		...
		private static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = BurstCompiler.CompileFunctionPointer<__codegen__OnCreate_000057C6_0024PostfixBurstDelegate>(delegate(IntPtr self, IntPtr state)
				{
					Invoke(self, state);
				}).Value;
			}
	...
	private EntityQuery __query_1943066277_0;

	private EntityQuery __query_1943066277_1;

	[BurstCompile]
	public void OnCreate(ref SystemState state)
	{
		state.RequireForUpdate<EndSimulationEntityCommandBufferSystem.Singleton>();
		state.RequireForUpdate<Access_T8_Dots>();
	}

	public void OnUpdate(ref SystemState state)
	{
		EntityCommandBuffer ecb = __query_1943066277_1.GetSingleton<EndSimulationEntityCommandBufferSystem.Singleton>().CreateCommandBuffer(state.WorldUnmanaged);
		foreach (QueryEnumerableWithEntity<InternalCompilerInterface.UncheckedRefRW<Access_T8_Dots>, InternalCompilerInterface.UncheckedRefRW<InteractiveObj_Dots>, InternalCompilerInterface.UncheckedRefRW<AccessBase_Dots>, InternalCompilerInterface.UncheckedRefRW<PhysicsCollider>, LocalTransform> item6 in IFE_1943066277_0.Query(__query_1943066277_0, __TypeHandle.__IFE_1943066277_0_TypeHandle, ref state))
		{
			item6.Deconstruct(out var item, out var item2, out var item3, out var item4, out var item5, out var entity);
			InternalCompilerInterface.UncheckedRefRW<Access_T8_Dots> uncheckedRefRW = item;
			...
			if (!uncheckedRefRW.ValueRW.isInitialized)
			{
				...
				{
					uncheckedRefRW.ValueRW.ett_Trigger = state.EntityManager.Instantiate(uncheckedRefRW3.ValueRW.ett_AccessTriggerUD);
					if (uncheckedRefRW3.ValueRW.Dir == FourDir.Up)
					{
						InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW.ValueRW.ett_Trigger).ValueRW.Position = localTransform.Position + new float3(0f, 1.5f, 0f);
					}
					else
					{
						InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__Unity_Transforms_LocalTransform_RW_ComponentLookup, ref state, uncheckedRefRW.ValueRW.ett_Trigger).ValueRW.Position = localTransform.Position + new float3(0f, -1.5f, 0f);
	...
	private void ApplyOffset(ref SystemState state, RefRW<Access_T8_Dots> access)
	{
		if (access.ValueRW.ett_Offset != Entity.Null)
		{
		...
		if (access.ValueRW.ett_OffsetNotNeedKey != Entity.Null)
		{
			InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__MatOverrideOffset_RW_ComponentLookup, ref state, access.ValueRW.ett_OffsetNotNeedKey).ValueRW.offset = new float2(0f, access.ValueRW.openCurrentYOffset);
		}
		if (access.ValueRW.ett_AccessLight != Entity.Null)
		{
			InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__MatOverrideOffset_RW_ComponentLookup, ref state, access.ValueRW.ett_AccessLight).ValueRW.offset = new float2(0f, access.ValueRW.openCurrentYOffset);
		}
		if (access.ValueRW.ett_AccessLightNotNeedKey != Entity.Null)
		{
			InternalCompilerInterface.GetComponentRWAfterCompletingDependency(ref __TypeHandle.__MatOverrideOffset_RW_ComponentLookup, ref state, access.ValueRW.ett_AccessLightNotNeedKey).ValueRW.offset = new float2(0f, access.ValueRW.openCurrentYOffset);
		}
	}
	...
	private void __AssignQueries(ref SystemState state)
	{
		...
		entityQueryBuilder2 = entityQueryBuilder2.WithAllRW<Access_T8_Dots>();
	...
	{
		__codegen__OnCreate_000057C6_0024BurstDirectCall.Invoke(self, state);
	}
	...
	{
		((Access_T8System*)self.ToPointer())->OnCreate(ref *(SystemState*)state.ToPointer());
	}
}
