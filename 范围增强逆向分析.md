...
| 弹体碰撞盒 | **不自动吃** | 预制体碰撞体，不读 `Calculate()` | 奥爆 / 冲刺等会把本体 `Scale` 写成 `Calculate()`；**黑洞不是这种**——伤害圈和特效读 `Radius.Calculate()`，本体 `LocalTransform.Scale` 保持预制体默认值。见下段与《黑洞逆向分析.md》7.2 |
...
| 激光 `1004` | **看形态**：非坠落废槽 / 坠落态是唯一伤害源 | 非坠落射线沿线检测**完全不读** `Radius`；坠落态用 `Radius.Calculate()` 当落点爆炸圈与折射搜敌圈 | 激光配置 `radius=0`，加算 `0×(1+0.3)=0` 仍是 0 → 非坠落是**纯废槽**；坠落激光沿线不检测单位，伤害全靠落点圈，反而**必须**靠这张卡（详见《激光逆向分析.md》§4.5 / §7.3） |
| 黑洞 `1007` | **吃，而且膨胀段也被加成** | DPS / 牵引 / 落地圈 / `ScaleMode.Radius` 特效都读 `Calculate()`；`Spell1007Job` 胀的是 `Radius.Base`，再被 `(1+AddRatio)×MulRatio` 乘一遍 | 本体 `LocalTransform.Scale` **不**改。`[范Lv1] [黑洞Lv3]` 出生 2.925，5 s 后 5.525，不是只大出生那一圈。聚爆 3113 对 Lv2/3 还有专属转伤通道。见《黑洞逆向分析.md》2.4 / 3.4 / 7.2 |
| 有「单次命中上限」的法术（落雷阵 `1018`） | **吃，但收益在上限处截断** | `Calculate()` 每 tick 现算，圈确实变大；可 `int1` 限死了一次结算最多打几个 | 圈里站 15 个也只打 `int1` 个，且是 `OverlapSphere` 返回顺序的前 `int1` 个。见 4.10 |
| 分解射线 `1011` | 看用法 | 部分折线 / 落地探测用 `Calculate()` | `radius=0` 时这段就是 0，加算乘上去仍是 0 |
...
4.1 的默认心智模型是「圈变大 = 打到的人变多」。**对带「单次命中上限」字段的法术，这条不成立：圈继续变大，命中数在上限处平掉。**

落雷阵（`1018` / `ThunderAura`）是目前已知的例子，上限写在它的 `int1` 上（3 / 5 / 7），每次结算取：

```175:175:decompiled/Assembly-CSharp/Spell1018ThunderAuraSystem.cs
			int num = math.min(valueRW.Int1, entities.Length);
```

`int1` 不进任何加算池，也没有任何符文改得动它。半径扩到 7 米、圈里站着 15 个敌人时，一次 tick 仍然只打 7 个——而且是 `PhysicsWorld.OverlapSphere` 返回顺序里的**前 7 个**，既不是最近的也不是随机的。

于是范围增强对落雷阵的真实收益只剩三条：

1. **把「凑满 `int1` 个目标」的概率推向 1。** 这是主要收益，对 Lv1（`int1 = 3`）价值最高；对 Lv3（`int1 = 7`）在小房间里边际收益迅速衰减，因为圈里本来就凑不出 7 个人。
2. **让敌人更难走出光环**，等价于变相延长有效时长。
3. **顺带放大另外两处读同一个半径的地方**：坠落态每跳的步长，以及分裂子光环的铺开环半径。

**它不提高单目标 DPS，也不能突破命中上限。** 打 Boss 时这张卡对落雷阵的收益接近 0。

数值（落雷阵 Lv3，`Base = 3.2`，不算遗物和法杖）：


| 槽位 | 半径 | 面积 | 传导环外径（×1.5） |
| --- | --- | --- | --- |
| `[落雷阵Lv3]` | 3.20 m | 32.2 m² | 4.80 m |
| `[范Lv1(+30%)] [落雷阵Lv3]` | 4.16 m | 54.4 m²（×1.69） | 6.24 m |
| `[范Lv2(+60%)] [落雷阵Lv3]` | 5.12 m | 82.4 m²（×2.56） | 7.68 m |
| `[范Lv3(+120%)] [落雷阵Lv3]` | 7.04 m | 155.7 m²（×4.84） | 10.56 m |
| `[范Lv3]×2 [落雷阵Lv3]` | 10.88 m | 371.9 m²（×11.6） | 16.32 m |


仍是 2.3 那条加算规则：两张 Lv3 是 `(1 + 1.2 + 1.2) = ×3.4`，不是 `2.2 × 2.2 = 4.84`。

**附带一条：半径在落雷阵身上是四重身份**——结算半径、传导环带的底数（硬编码 ×1.5）、坠落态每跳的步长、分裂子光环的生成环半径。任何改半径的东西都会四路同时生效，这在全表是独一份，复刻时不要假设「半径只影响伤害圈」。详见《落雷阵逆向分析.md》7.2 / 7.12。
...
| 落雷阵 `1018` | 圈照常放大，但 `int1`（3/5/7）是单次结算的命中上限，收益在上限处截断；半径在它身上是**四重身份**（结算圈 / 传导环底数 / 坠落步长 / 分裂环半径），见 4.10 |
...
| `Spell1018ThunderAuraSystem.cs` L175 | `min(Int1, count)`：命中上限截断扩圈收益 |
| `Spell1018ThunderAuraSystem.cs` L274、L847 | 同一个半径又当传导环底数（×1.5）、又当坠落每跳步长 |
...
5. **命中上限法术「扩圈后打谁」未实测**：4.10 只确认了 `min(int1, count)` 这道钳位。圈里人数超过上限时取的是 `PhysicsWorld.OverlapSphere` 的返回顺序（broadphase 树序），既不是最近也不是随机，是否稳定需要实机确认。另外「有命中上限」的法术目前只逐条确认了落雷阵 `1018`，没有全表扫过还有哪些法术带这类字段。
