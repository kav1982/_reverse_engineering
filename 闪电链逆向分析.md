...
### 5.3 锚点运动：链的形状由两端法术自己决定

Effect 里存的是 `SourceEntity` / `TargetEntity` 两个**实体**，不是两个坐标。Job 每帧重新取两端的 `LocalTransform` 算中点、朝向、长度（`Spell3007Job.cs` L179–189），下一次 `SphereCast` 也直接用当帧的两端坐标。**两端在动，链就在动；两端不动，链就是一条钉在地上的线段。**


| 两端形态 | 链的表现 |
| --- | --- |
| 两发普通飞弹（绝大多数情况） | 两颗弹朝不同方向飞，链迅速拉长、扫过一大片，几秒内因端点销毁而消失；命中窗口靠「扫到谁」 |
| 一发飞弹 + 一发静止 | 链绕着静止端扫扇形 |
| 两发静止法术 | **固定线段**，位置和长度全程不变 |


**落雷阵 `1018` 是第三种最干净的例子**：`speed = 0`、`duration = 5`，生成在枪口后原地不动整整 5 秒，不需要任何符文配合就是静止锚点。（悬停 `3008` 也能让飞弹在寿命末停住，但那要等飞行段走完，且停住的时间由 `HoverDuration` 决定。）

前提是要有第二个锚点。落雷阵 `shootCount = 1`，一次施放只出一个实体，第一发只会更新 `_lastSpellEntity`（4.1）。另一端得由**多重施法 `3002`**、**齐射同帧的另一张可施放**，或**法杖里的下一发**提供。一旦成立，两个光环之间就挂着一条不动的线段，每 `0.06` 秒沿它做一次 `SphereCast(width = 0.4)`。

`[多重施法Lv1] [闪电链Lv3] [落雷阵Lv3]`：两个静止光环 + 一条基础伤害 48 的固定线段。

**但「持续 5 秒的墙」这个说法要打个折扣**：链的寿命由两个条件先到先算——任一端实体消失（光环 5 秒到点销毁），或 `PenetrateCount` 用尽。而 `PenetrateCount` 基础只有 **1**（5.1），也就是说**没有穿透符文时，线段第一次打到人就消失了**，一共只结算 48 点伤害。同一目标也不是免费重复：命中后进 `0.1` s 冷却，冷却过去可以再打，但每次都要再扣一点 `PenetrateCount`。

所以静止锚点真正改变的不是伤害量，而是**这份预算什么时候被花掉**：

- 对飞弹的链：`PenetrateCount` 那几次结算必须靠「链此刻正好扫过敌人」，扫空了就随端点销毁一起浪费。
- 对落雷阵的链：线段守在原地等，敌人走进来才扣预算，**几乎不会浪费**。

想把它当墙用，必须同段叠穿透（`1 + Σ穿透 int1`）把预算抬上去。落雷阵 + 闪电链是一个机制上的奇观，不是 DPS 手段。详见《落雷阵逆向分析.md》7.11。
...
| 静止法术（落雷阵 `1018`） | 锚点不动 → 链是**固定线段**；需要多重/齐射/下一发提供另一端；预算不浪费但基础仍只有 1 次结算（5.3） |
