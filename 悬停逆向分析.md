...
> 配套：总览见《魔法工艺法术系统逆向报告.md》，原始数值见《法术数值总表.md》，槽位解析骨架见《伤害强化逆向分析.md》，坠落短路见《坠落逆向分析.md》，第二段结束才分裂见《分裂逆向分析.md》，悬停圈乘半径见《范围增强逆向分析.md》，「悬停期继续满速输出」的已核验例见《落雷阵逆向分析.md》与《黑洞逆向分析.md》7.4。
...
| 落雷阵 `1018` | **走**，且**悬停期继续全速输出** | `Duration` 用尽 → `Speed=0` → 加 `HoverTimer`，实体不销毁；而它的结算 Job **完全不看生命周期**，照样每 0.25 s 劈一次、伤害一分不减 | 「悬停 ≡ 时长强化」的已核验例之一，与时长强化可叠加，见下方展开 |
| 黑洞 `1007` | **走**，且**悬停期继续全速输出** | `Spell1007Job` 不读 `DurationTimer` / `HoverTimer`；DPS、牵引、胀圈、聚爆转伤全部继续 | 第二例。`[悬停Lv3] [黑洞Lv1]` = 5+5 s、40 tick。见《黑洞逆向分析.md》7.4 |
...
对比一下这张卡在别处的性价比：对飞弹是纯亏（停止推进、只剩一个不动的碰撞盒），对滚球是小赚，对冲刺是买无敌时间。**能把悬停秒数一比一换成伤害的，目前核验过落雷阵和黑洞。**
...
| `isDPS` / 自带 `DamageInterval`（黑洞 `1007`） | **确证继续转**。`Spell1007Job` 不读 `DurationTimer` / `HoverTimer`，悬停期 DPS、牵引、胀圈、聚爆转伤全部继续。`[悬停Lv3] [黑洞Lv1]` = 5 s 爬行 + 5 s 驻扎，单目标从 20 tick 变成 40 tick。见《黑洞逆向分析.md》7.4 |
...
| 落雷阵 `1018` | **悬停 ≡ 时长强化**：结算 Job 不设生命周期门，悬停期同频同伤继续输出，蓝耗不变；与 3103 可叠加。见 4.2 展开 |
| 黑洞 `1007` | **悬停 ≡ 时长强化**：`Spell1007Job` 同样不设门，悬停期 DPS / 牵引 / 胀圈继续。见《黑洞逆向分析.md》7.4 |
