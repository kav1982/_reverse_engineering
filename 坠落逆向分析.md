...
| 黑洞 `1007` | 坠落时三速 ×6 **不是主要影响**。`Spell1007Job` 在胀圈之后遇 `IsFallSpell` 立刻 `return`：途中**不 DPS、不牵引**，落地走同一个 `NewInfo` 仍 ×0.25，砸一下就销毁。详见下段与《黑洞逆向分析.md》5 / 7.3 |
...
**黑洞补充：`×6` 同样不是主要影响。** `Spell1007Job` 在胀圈、聚爆转伤之后的守卫是 `if (movement.IsFallSpell) return;`（`Spell1007Job.cs` L189–192）。坠落黑洞于是：

- **不圈伤**——`DamageTimer` 仍在加，但搜敌整段跳过
- **不牵引**——`SpellTakeDamageResultSystem` 的 `Float2 × DamageInterval` 拉人只挂在伤害结果上，不出伤就没有
- 圈**继续胀**（`Radius.Base += Float1/2 · dt` 在 `return` 之前）
- 落地 `Spell1007FallJob` 用同一个 `NewInfo`，`DamageInterval=0.25` 仍然生效，一击只有表伤的 **25%**（Mono 残留轨才是满额）
- 落地即销毁，不会变成地面光环

即「坠落 + 黑洞」= 一颗 ×6 速的坠落弹，砸一下出 0.25× 表伤的圈，放弃了这张卡几乎全部的持续输出。玩家侧看到的是「黑洞不吸人了」。详见《黑洞逆向分析.md》5 / 7.3。
