...
**对 `angle = 0` 且 `shootCount = 1` 的法术，钳零不是塌缩，是彻底空转。** 落雷阵 `1018`、黑洞 `1007` 都是 `angle = 0`、`shootCount = 1`（落雷阵还 `speed = 0`）：
...
| 黑洞 `1007` | 1 目标 × 20 tick = **20**（圈里 N 人则 ×N，不封顶） | 单目标期望 8 次；总量等价于整段 ×1.4 |
...
| 黑洞 `1007` | 同属 `angle=0` 空转；只剩暴击率。单目标 20 tick、圈里 N 人不封顶。见 2.6 与《黑洞逆向分析.md》7.6 |
